...
const LogoOptionsContainer = styled.ul`
  padding: 10px;
  margin: 0 auto;
  margin-top: 5vw;
  display: flex;
...
const ArkAnimation: React.FC = (): JSX.Element => {
  // 准备logo数据
  const logos = [
  ...
  useEffect(() => {
    if (canvas.current) {
      ...
      particleCanvas = new ParticleCanvas(canvas.current);
        ...
        {logoImgs.map((item, index) => {
          return (
            <LogoItem key={index}>
              <ItemPicture
                src={item.src}
                onClick={() => clickLogo(particleCanvas, item)}
              />
              <ItemDesc>{item.name}</ItemDesc>
            </LogoItem>
          );
        })}
      </LogoOptionsContainer>
